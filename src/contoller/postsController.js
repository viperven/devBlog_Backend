...
const createPost = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ isSucess: true, message: "Post created successfully", apiData: result });
...
const editPostById = async (req, res,next) => {
    try {
        ...
        const result = await Posts.findByIdAndUpdate(postId, { content, image }, { new: true });

        res.status(200).json({ isSuccess: true, message: "Post updated successfully", apiData: result });
    }
    catch (err) {
        console.log(err?.stack);
        next(err);
...
module.exports = { createPost,editPostById };